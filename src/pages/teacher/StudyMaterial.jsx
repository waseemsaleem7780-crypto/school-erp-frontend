...
const TeacherStudyMaterial = () => {
    ...
    const fetchData = async () => {
        try {
            const [c, tch] = await Promise.all([api.get('/classes/'), api.get('/teachers/')]);
            setClasses(c.data);
    ...
    // ✅ FIX: Sahi endpoint — /subjects/class/{id}
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setMessage({ type: 'success', text: `${t.studyMaterial} saved! ✅` });
    ...
    return (
        <div style={{ padding: '40px', fontFamily: 'Arial, sans-serif' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', flexWrap: 'wrap', gap: '16px' }}>
                <div>
                    <h1 style={{ fontSize: '32px', color: '#1a202c', margin: '0 0 8px 0' }}>{t.studyMaterial}</h1>
                ...
                >
                    {showForm ? '✕ Cancel' : `+ Upload ${t.studyMaterial}`}
                </button>
            </div>
            ...
            {showForm && (
                <div style={{
                    ...
                    <h3 style={{ marginTop: 0, color: '#1a202c' }}>Upload {t.studyMaterial}</h3>
                    <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
                        ...
                        <div style={{ gridColumn: '1 / -1' }}>
                            <button type="submit" disabled={loading || !form.file_url} style={{ padding: '14px 32px', fontSize: '15px', fontWeight: '600', color: 'white', background: (loading || !form.file_url) ? '#a0aec0' : '#48bb78', border: 'none', borderRadius: '10px', cursor: (loading || !form.file_url) ? 'not-allowed' : 'pointer' }}>
                                {loading ? 'Saving...' : `💾 Save ${t.studyMaterial}`}
                            </button>
                        </div>
                    </form>
                </div>
            ...
            <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '20px 24px', marginBottom: '24px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
                <label style={{ display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: '600', color: '#4a5568' }}>🔍 View {t.studyMaterial} by {t.class}</label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '16px' }}>
                {materials.length === 0 ? (
                    <div style={{ gridColumn: '1 / -1', backgroundColor: 'white', borderRadius: '16px', padding: '60px 20px', textAlign: 'center', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
                        ...
                        <p style={{ color: '#718096' }}>No {t.studyMaterial.toLowerCase()} yet</p>
                    </div>
                ) : (
                    materials.map((m) => (
...
export default TeacherStudyMaterial;import { useState, useEffect } from 'react';
import api from '../../api/axios';
import { useTerms } from '../../utils/terminology';
