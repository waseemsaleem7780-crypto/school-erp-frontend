...
const Sections = () => {
    ...
    const handleDelete = async (id) => {
        if (!window.confirm('Kya aap waqai ye section delete karna chahte ho?')) return;
            ...
            {showForm && (
                <div style={{
                    ...
                    <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
                        <div>
                            <label style={{ display: 'block', marginBottom: '6px', fontSize: '13px', fontWeight: '600', color: '#4a5568' }}>Section Name</label>
                            <input
                                type="text"
                                value={form.name}
            ...
            <div style={{ backgroundColor: 'white', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', overflow: 'hidden' }}>
                ...
                {sections.length === 0 ? (
                    <div style={{ padding: '60px 20px', textAlign: 'center' }}>
                        ...
                        <p style={{ color: '#718096' }}>No sections found</p>
                    </div>
                ) : (
